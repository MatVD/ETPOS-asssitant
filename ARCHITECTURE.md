...
Dans les deux cas, l'historique utile est reconstruit par l'application depuis `app.db` et fourni dans le prompt. Chaque passage transmet aussi à Codex son type documentaire, sa version détectée et sa date de révision lorsqu'elles existent. Le prompt rappelle explicitement que le manuel officiel (`manual`) reste la source de vérité fonctionnelle : une source complémentaire peut combler une absence du manuel mais ne doit pas remplacer silencieusement une procédure ou une règle que le manuel documente explicitement. La question courante, déjà persistée avant la génération, est retirée de l'historique réinjecté afin de ne pas la dupliquer. Pour les relances courtes, le retrieval peut réutiliser le dernier tour utilisateur pertinent sans second appel LLM. Les anciens marqueurs `[Sx]` sont retirés avant réinjection : seuls les identifiants de sources du tour courant sont citables.
...
Le navigateur peut interrompre une génération via l'annulation de la requête SSE. La question utilisateur, déjà persistée avant la génération, reste dans `app.db`, mais aucun fragment assistant interrompu n'est enregistré : seule une réponse ayant atteint l'événement final `done` peut être persistée comme réponse documentaire. Le retry est autorisé uniquement lorsque le dernier message persistant de la conversation est cette question utilisateur sans réponse ; il la réutilise sans insérer un second message utilisateur. L'annulation ferme également le runtime Codex du tour en cours afin de ne pas laisser de génération orpheline.
...
La dictée est une couche d'entrée indépendante du RAG et de Codex. Le navigateur capture le microphone uniquement après action explicite avec `getUserMedia()` et `MediaRecorder`, puis envoie l'audio brut à `POST /api/transcribe`. Les formats navigateur sont négociés dynamiquement afin de couvrir notamment WebM/Opus et MP4 sans imposer un codec unique.

Le backend utilise `faster-whisper` avec Whisper `large-v3-turbo`, langue française forcée, CPU `int8` par défaut et Silero VAD. Le modèle est chargé paresseusement une seule fois par worker puis réutilisé. Les transcriptions sont sérialisées dans le worker afin d'éviter plusieurs inférences lourdes concurrentes sur le VPS CPU. Le vocabulaire de `config/transcription_hotwords.txt` est transmis comme `hotwords` : il ne contient que des termes ou expressions métier utiles à la reconnaissance et ne constitue jamais une source de réponse.
...
La seconde couche, `eval-answer`, utilise le provider Codex et réemploie le même retrieval, le même prompt et la même finalisation de réponse/citations que le chat de production. Elle mesure de manière déterministe : abstention correcte, statut documentaire réellement produit (`full`, `partial` ou `none`), couverture des faits obligatoires, exactitude des chemins de menus attendus, présence des citations, couverture des groupes de passages attendus par les citations, part des citations situées dans ces passages attendus, soutien lexical des faits/chemins par les passages cités et latence de réponse complète. Les faits peuvent déclarer plusieurs formulations textuelles équivalentes et les chemins de menus sont comparés comme des séquences ordonnées d'étapes afin d'éviter les faux négatifs de simple formulation ou mise en forme. La couverture des passages attendus et le soutien des faits/chemins sont bloquants en mode `--strict` ; la part des citations dans ces passages reste secondaire, car des citations supplémentaires peuvent légitimement soutenir des précisions complémentaires. Aucun de ces indicateurs n'est une mesure sémantique complète du soutien de chaque affirmation par la source. `--docs-db` permet d'exécuter cette évaluation directement contre une candidate non activée. L'abstention complète utilise une réponse canonique afin de pouvoir être scorée sans juge LLM. `make quality-baseline` lance les campagnes Codex avec `--strict`, de sorte qu'un échec déterministe fait réellement échouer la Quality Gate locale. La commande peut aussi produire un rapport JSON détaillé contenant la réponse, les sections récupérées, les citations, les latences, le modèle, l'effort de raisonnement, la verbosité, le SHA Git et les métriques provider/tokens pour permettre une revue humaine et des comparaisons reproductibles.

Un jeu `eval/acceptance.jsonl`, distinct du benchmark de développement, contient des formulations naturelles qui ont initialement révélé plusieurs lacunes de généralisation du vocabulaire. `eval/challenge.jsonl` complète cette couverture avec douze suivis conversationnels : relances courtes, pronoms, corrections de référent, changements de sujet et retour à un thème antérieur. Après cette première utilisation diagnostique, il est conservé comme suite de régression d'acceptation ; de futurs cas réellement aveugles devront provenir de nouvelles questions terrain et ne pas être utilisés pour régler le retrieval avant leur première mesure. `eval/support.jsonl` sert de contrat de source pour les 10 FAQ françaises officielles. `eval/news.jsonl` sert de contrat de source pour l'actualité Verifone et couvre les trois faits métier principaux retenus avant activation.

Le taux d'hallucination n'est pas calculé automatiquement : il nécessite une revue humaine ou un protocole de juge distinct dont la fiabilité aura été validée. La Quality Gate locale peut bloquer sur les critères déterministes de `eval-answer --strict`, mais le déploiement automatique continue de ne bloquer que sur l'évaluation retrieval, rapide et déterministe ; l'évaluation Codex reste volontairement séparée du CI/CD.

Baseline locale du corpus actif au 22 septembre 2026, K=5, 47 cas : Recall@5 100,0 %, MRR 0,884, couverture des groupes 100,0 %. Le corpus actif contient 3 documents / 273 sections : manuel ETPOS V5.34, Support français et actualité officielle Verifone. L'acceptance atteint Recall@5 100,0 %, MRR 0,692 et couverture 100,0 %. Le challenge atteint Recall@5 100,0 %, MRR 0,809 et couverture 100,0 % sur 49 cas répondables. Le contrat Support atteint Recall@5 100,0 %, MRR 0,643 et couverture 100,0 % sur 10 FAQ ; le contrat Actualités atteint Recall@5 100,0 %, MRR 0,667 et couverture 100,0 % sur 3 cas. Le benchmark couvre explicitement l'ambiguïté générique « Comment ajouter un nouveau compte ? » : le top 5 doit désormais faire remonter les trois sens métier pertinents (compte de vente, compte courant client, utilisateur/opérateur) au lieu d'en choisir silencieusement un seul.
