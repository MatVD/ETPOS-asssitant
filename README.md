...
Un premier pilote d'actualités officielles est déclaré mais reste désactivé : l'article français consacré à l'intégration ETPOS-Verifone. Il utilise le profil `news_article`, une priorité inférieure au Support et le contrat `eval/news.jsonl`. Les actualités restent déclarées article par article ; le projet ne crawle pas automatiquement le blog. Tant que le pilote n'a pas passé `inspect-source`, les benchmarks retrieval de candidate et une revue des réponses Codex, il n'entre pas dans le corpus actif ni dans `make quality-baseline`.
